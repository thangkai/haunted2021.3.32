...
public class Health_Bar : MonoBehaviour
{
    ...
    [SerializeField] float m_Max_Health_Fake;
    [SerializeField] float m_Curr_Health_Fake;
    ...
    [SerializeField] Transform m_Progress_Health_Fake;
    [SerializeField] GameObject m_Obj_Bg_Fake;
    [SerializeField] GameObject m_Obj_Progress_Fake;


    private void Start()
    {
        m_Exp_Bar = GetComponent<Exp_Bar>();
        m_Is_Die = false;
    }
    ...
    public float Get_Cur_Health()
    {
        return m_Curr_Health;
    }
    ...
    public void Set_Is_Full_Health_Skill_Safeguard()
    {
        m_Max_Health_Fake = Get_Max_Health();
        m_Curr_Health_Fake = m_Max_Health_Fake;
    }
    public void Set_Is_Health_After_Time_Skill_Safeguard()
    {
        m_Max_Health_Fake = 0f;
        m_Curr_Health_Fake = m_Max_Health_Fake;
    }
    ...
    public void Set_Show_Progress_Fake(bool is_Show)
    {
        if (m_Is_Auto_Hide_Health_Bar && !m_Is_Die)
        {
            m_Obj_Bg_Fake.Set_Active(is_Show);
            m_Obj_Progress_Fake.Set_Active(is_Show);
            //Check_Init_Exp_Bar();
            //m_Exp_Bar.Set_Show_Bar(is_Show);
        }
    }
    ...
    public void Set_Force_Show_Progress_Fake(bool is_Show)
    {
        m_Obj_Bg_Fake.Set_Active(is_Show);
        m_Obj_Progress_Fake.Set_Active(is_Show);
    ...
    public void Increae_Max_Health(float new_Max_Health, bool is_Force_Full_Health = true)
    {
        ...
        Debug.Log(new_Max_Health + "maxhealth");
    ...
    {
        if (m_Curr_Health_Fake > 0)
        {
            Set_Show_Progress_Fake(true);
            m_Curr_Health_Fake -= damage;
            if (m_Curr_Health_Fake <= 0)
            {
                m_Curr_Health_Fake = 0;

                Set_Show_Progress_Fake(false);
    ...
    float m_Tmp_Progress_Fake;
    private void Update_Progress_Fake()
    {
        if (m_Is_Die) return;
        m_Tmp_Progress_Fake = m_Curr_Health_Fake / m_Max_Health_Fake;
        m_Progress_Health_Fake.localScale = new Vector3(m_Tmp_Progress_Fake, 1, 1);

        if (m_Is_Auto_Hide_Health_Bar)
        {
            Set_Show_Progress_Fake(m_Curr_Health_Fake < m_Max_Health_Fake);
        }
    }
    ...
    {
        m_Progress_Health.localScale = new Vector3(0, 1, 1);
