...
//   [SerializeField] NavMeshSurface m_Nav_Mesh;
    ...
    public void Set_Door_Empty(Transform door, [Bridge.Ref] Vector3 pos)
    {
        if (m_Room_Tut)
        {
            m_Door_Empty = door;
    ...
    public Vector3 Get_Nearest_Point_Health([Bridge.Ref] Vector2 pos_Boss)
    {
    ...
    public bool Check_Exist_Pos_Line_Up_In_Other_Room(int id_Room, [Bridge.Ref] Vector3 pos)
    {
        for (int i = 0; i < m_All_Room_Has_Player.Count; i++)
        {
