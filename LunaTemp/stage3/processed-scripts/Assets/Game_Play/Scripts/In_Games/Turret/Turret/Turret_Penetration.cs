...
public class Turret_Penetration : Base_Turret_Can_Upgrade
{
    ...
    [SerializeField] protected float m_time_Dame_Penetration;
   ...
   [SerializeField] Bool_Global_Variable m_Live_Data_Item_SP_Speed_X2;
    ...
    public override void Update_Model_Price()
    {
        ...
        m_Range = model_Info_Turret_Curr.model_Skills.range + 1;// toc danh

        m_Damage = model_Info_Turret_Curr.model_Skills.damage;// dame
        m_CD = model_Info_Turret_Curr.model_Skills.speed;// toc danh
        m_Penetation = model_Info_Turret_Curr.model_Skills.damage_Penetration;
        m_time_Dame_Penetration = model_Info_Turret_Curr.model_Skills.time_Dame_Penetration;
        Debug.LogError("Xekotoby" + " m_Range" + m_Range + " m_Damage : " + m_Damage + " m_CD" + m_CD + "m_Penetation" + m_Penetation + "m_time_Dame_Penetration" + m_time_Dame_Penetration);
        //xekotoby
        //them dame xuyen giap
        // dame xuyen giap cua sung bth auto
        // sau khi turretPenetation bắn
        // thì dame xuyên giáp  == dame dame xuyen giap turret penetation cung cấp
        // khi hết time
        // trả về là 0 
        // trc đó fai tạo súng xuyên giáp 
    ...
    public override void Update_Model_By_Level()
    {
        ...
        {
            m_Turret_Level_Penatation[0].Set_Active(false);
            m_Anim_Spine.gameObject.SetActive(true);
            m_Anim_Spine.skeletonDataAsset = m_Anim_Skeleton_Data_Asset;
            if (level_Curr == 1)
            {
                m_Anim_Spine.AnimationName = "u2";
            }
            else if (level_Curr == 2)
            {
                m_Anim_Spine.AnimationName = "u3";
            }
            else if (level_Curr == 3)
            {
                m_Anim_Spine.AnimationName = "u4";

            }
            else
            {
                Debug.LogError("xekotoby");
    ...
    public virtual void Spawn_Bullet()
    {
        ...
        GameObject obj_Bullet =
            Instantiate(Data_In_Game.Instance.Get_Data_Prefaps().Get_Pref_Bullet_Player_Penetation(), transform);
        ...
        {
            Bullet_Penetation_Controller bullet_Controller = obj_Bullet.GetComponent<Bullet_Penetation_Controller>();
            ...
            if (bullet_Controller)
            {
                ...
                bullet_Controller.Set_Target(transform.GetInstanceID(),
                    ...
                    m_Type_Bullet_Skill, Type_Turret.Turret_Pierce, m_Penetation, m_time_Dame_Penetration);
    ...
    public void Set_Charm([Bridge.Ref] Vector3 pos_Boss, bool is_Stun)
    {
        if (is_Stun)
        {
    ...
    {
        if (DataSaved.Get_Unlock_Skill_Tree_Player(Type_Skill_Tree.Skill_10_Damage_15_Per))
        {
            max_Health *= 1.2f;
        }
    ...
    private void Check_Data_Perk_Increase_Speed_Atk()
    {
        ...
        if (DataSaved.Get_Unlock_Skill_Tree_Player(Type_Skill_Tree.Skill_32_Speed_Attack_Turret_Peratation_75_Per))
        {
            m_CD *= 0.25f;
        }
        else if (DataSaved.Get_Unlock_Skill_Tree_Player(Type_Skill_Tree.Skill_21_Speed_Attack_Turret_Peratation_50_Pe))
        {
            m_CD *= 0.5f;
        }
        else if (DataSaved.Get_Unlock_Skill_Tree_Player(Type_Skill_Tree.Skill_5_Speed_Attack_Turret_Peratation_10_Per))
        {
            m_CD *= 0.9f;
        }
